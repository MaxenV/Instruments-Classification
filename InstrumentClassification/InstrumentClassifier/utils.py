...
def predict_class(audio_file, model):
    try:
        ...
        # Map predictions to class names using hardcoded mappings
        result = {}
        for i in range(len(predictions_percentage)):
            
            # Get the class name from the hardcoded mappings using the index i (as string)
            class_name = class_mappings.get(str(i), f"Unknown Class {i+1}")
            result[class_name] = f"{predictions_percentage[i]:.2f}%"
        
        # Sort results based on percentage in descending order
        sorted_result = dict(sorted(result.items(), key=lambda item: float(item[1].strip('%')), reverse=True))

        print("Mapped Predictions (sorted):", sorted_result)  # Debug: check mapped predictions
